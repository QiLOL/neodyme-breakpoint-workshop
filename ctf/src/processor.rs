...
impl Processor {
    ...
    pub fn process_pay_farm_fee(
        ...
        let farm_id_info = next_account_info(account_info_iter)?; // @audit the target farm
        let authority_info = next_account_info(account_info_iter)?; // @audit farm authority address -> PDA
        let creator_info = next_account_info(account_info_iter)?; // @audit who create the farm
        let creator_token_account_info = next_account_info(account_info_iter)?; // @audit creator's token account
        let fee_vault_info = next_account_info(account_info_iter)?; // @audit farm fee vault to receive the token
        let token_program_info = next_account_info(account_info_iter)?; // @audit token program
        let mut farm_data = try_from_slice_unchecked::<Farm>(&farm_id_info.data.borrow())?; //@audit will not check the length
        ...
        if *authority_info.key != Self::authority_id(program_id, farm_id_info.key, farm_data.nonce)? {
            msg!("the address: {}", authority_info.key);
